...
const NotificationsDropdown: React.FC<NotificationsDropdownProps> = ({
  ...
  const {
    ...
    queryFn: async () => {
      ...
      return getAdminNotificationsApi({ signal: controller.signal });
    },
  ...
  const handleMarkAsRead = useCallback(async (notificationId: string) => {
    try {
      ...
      queryClient.setQueryData(["admin-notifications"], (oldData: typeof notificationsResponse) => {
        if (!oldData?.data) return oldData;
        
        return {
          ...oldData,
          data: oldData.data.map((notification: IAdminNotification) =>
    ...
    try {
      ...
      await Promise.all(
        unreadNotifications.map(notification => 
          markNotificationAsReadApi(notification._id)
        )
      );
