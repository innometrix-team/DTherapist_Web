...
import { ChevronDownIcon, SearchIcon, VideoIcon, PhoneIcon, MessageCircleIcon } from "lucide-react";
...
const AdminBookingsTable: React.FC<AdminBookingsTableProps> = ({
  ...
  const totalPages = Math.ceil(totalBookings / (filters.limit || ITEMS_PER_PAGE));
    ...
    return (
      <div className={`bg-white p-4 sm:p-6 space-y-6 w-full ${className}`}>
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 text-center">
          ...
          <p className="text-yellow-500 text-xs mt-1">Admin access required to view bookings</p>
        </div>
      </div>
    );
  }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 px-4 sm:px-6 pt-4 sm:pt-6">
        <div className="flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-8">
          <div className="flex items-center space-x-2">
            ...
            <span className="font-semibold text-gray-900">{totalBookings.toLocaleString()}</span>
          </div>
          <div className="flex items-center space-x-2">
            <span className="text-gray-600 text-sm">Showing:</span>
            <span className="font-semibold text-gray-900 text-xs sm:text-sm">
              {Math.min((currentPage - 1) * (filters.limit || ITEMS_PER_PAGE) + 1, totalBookings)}-{Math.min(currentPage * (filters.limit || ITEMS_PER_PAGE), totalBookings)} of {totalBookings}
            </span>
          </div>
        </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex justify-between">
                      ...
                      <div className="flex items-center space-x-1">
                        <span className={`${getAdminSessionTypeColor(booking.sessionType)}`}>
                          {getSessionTypeIcon(booking.sessionType)}
                        </span>
                        <span className={`text-xs font-medium ${getAdminSessionTypeColor(booking.sessionType)}`}>
      ...
      {totalBookings > 0 && (
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 px-4 sm:px-6 pb-4 sm:pb-6">
          <div className="text-sm text-gray-500 text-center sm:text-left">
            Showing {Math.min((currentPage - 1) * (filters.limit || ITEMS_PER_PAGE) + 1, totalBookings)} to {Math.min(currentPage * (filters.limit || ITEMS_PER_PAGE), totalBookings)} of {totalBookings} results
          </div>
